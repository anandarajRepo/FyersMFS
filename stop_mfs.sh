...
echo "$(date): Attempting to stop MFS scalping process..." >> /var/log/mfs.log
...
# Log completion
echo "$(date): Stop script completed" >> /var/log/mfs.log

# Optional: Check if process is still running
if pgrep -f "main.py run" > /dev/null; then
    echo "$(date): WARNING - Process still running!" >> /var/log/mfs.log
else
    echo "$(date): Process successfully stopped" >> /var/log/mfs.log
fi