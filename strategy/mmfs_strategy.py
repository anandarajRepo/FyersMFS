...
class MMFSStrategy:
    ...
    def _print_final_metrics(self):
        ...
        logger.info(f"Gross P&L: Rs.{self.metrics.gross_pnl:+,.2f}")
        logger.info(f"Net P&L: Rs.{self.metrics.net_pnl:+,.2f}")
        logger.info(f"Profit Factor: {self.metrics.profit_factor:.2f}")
        logger.info(f"Expectancy: Rs.{self.metrics.expectancy:+,.2f}")
        logger.info(f"\nSetup Performance:")
